...
> Base URL: `http://localhost:8080` (local). All endpoints are prefixed `/api/v1`.
> Swagger UI is live at `/swagger-ui.html` (OpenAPI JSON at `/v3/api-docs`) — use this document for
> the full picture (security model + validation rules), and Swagger to try requests live.
>
> This document was generated by reading the actual controller/DTO/security source in
> `az.testifyaz.backend`, not from design docs — it reflects exactly what the code does today.
...
ScoringMode:          PERCENTAGE, EXAM_WEIGHTED
...
NotificationType:     RESULT_READY, CHILD_TEST_COMPLETED
```
...
### `POST /students/me/link-codes`
Generates a single-use, time-limited code for a parent to redeem (§9).
**Response `201` (`ParentLinkCodeResponse`):** `{ "code": "string", "expiresAt": "instant", "consumed": false, "createdAt": "instant" }`
...
### `POST /parents/me/links`
Redeem a code the student generated (§8) to create the link.
**Body (`RedeemCodeRequest`):** `{ "code": "required" }`
**Response `201` (`LinkedChildResponse`):** `{ "studentId": "uuid", "linkedAt": "instant" }`
Errors: invalid code → 404; already-used code → 409; expired code → 422 (`BUSINESS_RULE_VIOLATION`).

### `DELETE /parents/me/links/{studentId}`
Revokes the link — immediately cuts off all access to that child's data. **Response `204`.**
A fresh code can re-establish the link later (reactivates the same relationship row).
...
### `POST /admin/questions/{id}/approve` → `ACTIVE`. **Response `204`.**
### `POST /admin/questions/{id}/reject` → `REJECTED`. **Response `204`.**
### `POST /admin/questions/{id}/archive` → `ARCHIVED`. **Response `204`.**

These enforce a state machine server-side: approve/reject only work from `DRAFT`/`PENDING_REVIEW`;
archive works from any non-`ARCHIVED` state; a same-status call is a harmless no-op; any other
transition (e.g. approving an already-`ARCHIVED` question) returns `409`.
