    ...
    <div className='px-24 py-24'>
      <div className='bg-white rounded-10 px-24 py-24 mb-24'>
        <div className='d-flex flex-wrap align-items-center justify-content-between gap-16 mb-24'>
          <div>
            <h4 className='fw-semibold text-neutral-500 text-20 mb-4'>Learner progress</h4>
            <p className='text-14 text-neutral-400 mb-0'>Results and cross-test trends for linked learners.</p>
          </div>
          <div className='d-flex flex-wrap align-items-center gap-10'>
            <select className='form-select rounded-pill border-neutral-40 text-14 py-11 px-16 min-w-220-px' value={selectedLearnerId} onChange={changeLearner}>
              {!learners.length ? <option value=''>No linked learners</option> : null}
        ...
        {selectedLearner ? (
          <div className='row gy-3'>
            <div className='col-md-4'><div className='border border-neutral-30 rounded-8 p-16'><span className='text-13 text-neutral-400 d-block'>Learner</span><strong className='text-17 text-neutral-500'>{selectedLearner.name || selectedLearner.learnerId}</strong></div></div>
            <div className='col-md-4'><div className='border border-neutral-30 rounded-8 p-16'><span className='text-13 text-neutral-400 d-block'>Recent average</span><strong className='text-17 text-neutral-500'>{selectedLearner.averagePercentage != null ? `${Math.round(selectedLearner.averagePercentage)}%` : "-"}</strong></div></div>
            <div className='col-md-4'><div className='border border-neutral-30 rounded-8 p-16'><span className='text-13 text-neutral-400 d-block'>Recent results</span><strong className='text-17 text-neutral-500'>{selectedLearner.recentResultCount || 0}</strong></div></div>
          </div>
        ) : <p className='text-14 text-neutral-400 mb-0'>Link a learner from the Children page to see progress.</p>}
      </div>

      <div className='row gy-4 mb-24'>
        <div className='col-xl-6'>
          <div className='bg-white rounded-10 px-24 py-24 h-100'>
            <h5 className='text-16 fw-semibold text-neutral-500 mb-16'>Subject trends</h5>
            {isLoading ? <p className='text-14 text-neutral-400 mb-0'>Loading...</p> : subjectTrends.length ? (
              <div className='table-responsive'>
                <table className='table mb-0'>
                  <thead><tr><th className='text-12 py-12'>Subject</th><th className='text-12 py-12'>Accuracy</th><th className='text-12 py-12'>Tests</th><th className='text-12 py-12'>Direction</th></tr></thead>
                  <tbody>
        ...
        <div className='col-xl-6'>
          <div className='bg-white rounded-10 px-24 py-24 h-100'>
            <h5 className='text-16 fw-semibold text-neutral-500 mb-16'>Topic trends</h5>
            {isLoading ? <p className='text-14 text-neutral-400 mb-0'>Loading...</p> : topicTrends.length ? (
              <div className='table-responsive'>
                <table className='table mb-0'>
                  <thead><tr><th className='text-12 py-12'>Topic</th><th className='text-12 py-12'>Accuracy</th><th className='text-12 py-12'>Questions</th><th className='text-12 py-12'>Direction</th></tr></thead>
                  <tbody>
      ...
      <div className='bg-white rounded-10 px-24 py-24'>
        <h5 className='text-16 fw-semibold text-neutral-500 mb-16'>Result history</h5>
        <div className='table-responsive admin-users-table'>
          <table className='table mb-0'>
            <thead><tr><th className='text-12 py-14 px-16'>Type</th><th className='text-12 py-14 px-16'>Score</th><th className='text-12 py-14 px-16'>Percentage</th><th className='text-12 py-14 px-16'>Correct</th><th className='text-12 py-14 px-16'>Scored</th><th className='text-12 py-14 px-16 text-end'>Action</th></tr></thead>
            <tbody>
              {isLoading ? <tr><td className='py-20 px-16 text-neutral-400' colSpan='6'>Loading...</td></tr> : results.length ? results.map((result) => (
                <tr key={result.id || result.sessionId}>
                  ...
                  <td className='py-14 px-16 text-end'><button type='button' className='px-14 py-8 border border-neutral-40 rounded-pill text-14 text-neutral-500 bg-white' onClick={() => openDetail(result.sessionId)}>Details</button></td>
                </tr>
              )) : <tr><td className='py-20 px-16 text-neutral-400' colSpan='6'>No results found.</td></tr>}
            </tbody>
          </table>
        </div>
        <div className='d-flex align-items-center justify-content-end gap-8 mt-20'>
          <button type='button' className='px-14 py-8 border border-neutral-40 rounded-8 text-14 text-neutral-500' disabled={isLoading || meta.page <= 1} onClick={() => changePage(Math.max(meta.page - 1, 1))}>Previous</button>
          <span className='text-14 text-neutral-400'>{meta.page} / {meta.totalPages}</span>
          <button type='button' className='px-14 py-8 border border-neutral-40 rounded-8 text-14 text-neutral-500' disabled={isLoading || meta.page >= meta.totalPages} onClick={() => changePage(Math.min(meta.page + 1, meta.totalPages))}>Next</button>
        </div>
      </div>
          ...
          <div className='modal-dialog modal-dialog-scrollable modal-xl' role='document'>
            <div className='modal-content rounded-12 border-0'>
              <div className='modal-header border-neutral-30'>
                <h5 className='modal-title text-18 fw-semibold text-neutral-500'>Learner result</h5>
                ...
                {isDetailLoading ? <p className='text-14 text-neutral-400 mb-0'>Loading...</p> : (
                  <>
                    <div className='row gy-3 mb-20'>
                      <div className='col-md-4'><div className='border border-neutral-30 rounded-8 p-14'><span className='text-12 text-neutral-400 d-block'>Score</span><strong>{detail.totalScore ?? "-"} / {detail.maxScore ?? "-"}</strong></div></div>
                      <div className='col-md-4'><div className='border border-neutral-30 rounded-8 p-14'><span className='text-12 text-neutral-400 d-block'>Percentage</span><strong>{detail.percentage != null ? `${Math.round(detail.percentage)}%` : "-"}</strong></div></div>
                      <div className='col-md-4'><div className='border border-neutral-30 rounded-8 p-14'><span className='text-12 text-neutral-400 d-block'>Correct / wrong / blank</span><strong>{detail.correctCount || 0} / {detail.wrongCount || 0} / {detail.blankCount || 0}</strong></div></div>
                    </div>
                    <div className='d-flex flex-column gap-12'>
                      {(detail.details || []).map((item, index) => (
                        <div className='border border-neutral-30 rounded-10 p-16' key={`${item.questionId}-${index}`}>
                          <div className='d-flex align-items-center gap-8 mb-10'><strong className='text-14 text-neutral-500'>Question {index + 1}</strong><AdminStatusBadge status={item.correct ? "ACTIVE" : item.blank ? "PENDING" : "REJECTED"} label={item.correct ? "Correct" : item.blank ? "Blank" : "Wrong"} /></div>
                          <div className='text-14 text-neutral-500 mb-10' dangerouslySetInnerHTML={renderQuestionHtml(item.stem || "")} />
                          <span className='text-13 text-neutral-400'>Learner answer: {item.studentAnswer || "-"} · Correct: {item.correctAnswer || "-"}</span>
                        </div>
                      ))}
                    </div>
                  </>
