    ...
    <div className='px-24 py-24'>
      <div className='bg-white rounded-10 px-24 py-24'>
        <div className='d-flex flex-wrap align-items-center justify-content-between gap-16 mb-24'>
          <div>
            <h4 className='fw-semibold text-neutral-500 text-20 mb-4'>Assignments</h4>
            <p className='text-14 text-neutral-400 mb-0'>Parent invitations and assigned exams appear here.</p>
          </div>
        ...
        {invitations.length ? (
          <div className='mb-24'>
            <h5 className='text-16 fw-semibold text-neutral-500 mb-12'>Parent invitations</h5>
              ...
              {invitations.map((invitation) => (
                <div className='border border-neutral-30 rounded-8 px-16 py-14 d-flex flex-wrap align-items-center justify-content-between gap-12' key={invitation.invitationId}>
                  ...
                  <div className='d-flex gap-8'>
                    <button type='button' className='btn btn-main rounded-pill px-18 py-8' onClick={() => respond(invitation.invitationId, true)}>Approve</button>
                    <button type='button' className='px-18 py-8 border border-neutral-40 rounded-pill text-14 text-neutral-500 bg-white' onClick={() => respond(invitation.invitationId, false)}>Reject</button>
                  </div>
                </div>
              ))}
            </div>
        ...
        <h5 className='text-16 fw-semibold text-neutral-500 mb-12'>Assigned exams</h5>
          ...
          <table className='table mb-0'>
            <thead>
              <tr>
                ...
                <th className='text-12 fw-medium text-neutral-500 py-16 px-20'>State</th>
              ...
              {isLoading ? (
                ...
                exams.map((exam) => (
                  <tr key={exam.examId || exam.examCode}>
                    <td className='py-16 px-20 text-14 text-neutral-500'>{exam.title || "Untitled exam"}</td>
                    ...
                    <td className='py-16 px-20'>
                      <div className='d-flex justify-content-end gap-8'>
                        <button type='button' className='px-14 py-8 border border-neutral-40 rounded-pill text-14 text-neutral-500 bg-white' onClick={() => openPreview(exam.examCode)}>Preview</button>
                        {exam.resumable && exam.sessionId ? (
                          <Link href={`/admin/exam-session/${exam.sessionId}`} className='btn btn-main rounded-pill px-18 py-8'>Continue</Link>
                        ) : exam.state !== "SUBMITTED" && exam.state !== "EXPIRED" ? (
                          <Link href={`/admin/exams/take/${exam.examCode}`} className='btn btn-main rounded-pill px-18 py-8'>Start</Link>
                        ) : null}
                      </div>
                    </td>
                  </tr>
                ...
                <tr><td className='py-20 px-20 text-neutral-400' colSpan='6'>No assigned exams found.</td></tr>
              )}
            </tbody>
          </table>
        </div>
          ...
          <div className='modal-dialog modal-dialog-centered modal-lg' role='document'>
            <div className='modal-content rounded-12 border-0'>
              <div className='modal-header border-neutral-30'>
                <h5 className='modal-title text-18 fw-semibold text-neutral-500'>Exam preview</h5>
                <button type='button' className='btn-close' aria-label='Close' onClick={() => setPreview(null)}></button>
              </div>
              <div className='modal-body'>
                {preview.isLoading ? <p className='text-14 text-neutral-400 mb-0'>Loading...</p> : (
                  <>
                    <h6 className='text-18 fw-semibold text-neutral-500 mb-8'>{preview.title || "Untitled exam"}</h6>
                    {preview.description ? <p className='text-14 text-neutral-400'>{preview.description}</p> : null}
                    <div className='row gy-3 mb-16'>
                      <div className='col-md-4'><div className='border border-neutral-30 rounded-8 p-14'><span className='text-13 text-neutral-400 d-block'>Questions</span><strong>{preview.totalQuestions ?? "-"}</strong></div></div>
                      <div className='col-md-4'><div className='border border-neutral-30 rounded-8 p-14'><span className='text-13 text-neutral-400 d-block'>Duration</span><strong>{preview.durationMinutes ? `${preview.durationMinutes} min` : "Untimed"}</strong></div></div>
                      <div className='col-md-4'><div className='border border-neutral-30 rounded-8 p-14'><span className='text-13 text-neutral-400 d-block'>Creator</span><strong>{preview.creatorName || "-"}</strong></div></div>
                    </div>
                    <p className='text-14 text-neutral-500 mb-8'>Subjects: {(preview.subjects || []).join(", ") || "-"}</p>
                    <p className='text-14 text-neutral-500 mb-0'>Topics: {(preview.topics || []).join(", ") || "-"}</p>
                  </>
                )}
              </div>
            </div>
