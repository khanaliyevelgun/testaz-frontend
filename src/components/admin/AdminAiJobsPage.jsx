    ...
    <div className='px-24 py-24'>
      <div className='bg-white rounded-10 px-24 py-24'>
        <div className='d-flex flex-wrap align-items-center justify-content-between gap-16 mb-24'>
          <div>
            <h4 className='fw-semibold text-neutral-500 text-20 mb-4'><StaticText text={"AI Jobs"} /></h4>
            <p className='text-14 text-neutral-400 mb-0'><StaticText text={"Generate questions and track asynchronous jobs."} /></p>
          </div>
        ...
        <form className='d-flex flex-wrap align-items-center gap-12 mb-24' onSubmit={handleSubmit}>
          ...
          <select name='questionType' className='form-select rounded-pill border-neutral-40 text-14 py-11 px-16 w-auto min-w-190-px' value={form.questionType} onChange={handleChange}>
            <StaticOption value='SINGLE_CHOICE' text={"SINGLE CHOICE"} />
        ...
        <div className='table-responsive admin-users-table'>
          <table className='table mb-0'>
            <thead>
              <tr>
                <th className='text-12 fw-medium text-neutral-500 py-16 px-20'><StaticText text={"Job"} /></th>
                ...
                <th className='text-12 fw-medium text-neutral-500 py-16 px-20'><StaticText text={"Count"} /></th>
              ...
              {isLoading ? (
                ...
                jobs.map((job) => (
                  <tr key={job.id}>
                    ...
                    <td className='py-16 px-20 text-end'><button type='button' className='px-14 py-8 border border-neutral-40 rounded-pill text-13 text-neutral-500 bg-white' onClick={() => openJob(job.id)}><StaticText text={"Details"} /></button></td>
                  </tr>
                ))
              ) : (
          ...
          <div className='modal-dialog modal-dialog-centered modal-lg' role='document'>
            <div className='modal-content rounded-12 border-0'>
              <div className='modal-header border-neutral-30'>
                <div><h5 className='modal-title text-18 fw-semibold text-neutral-500'><StaticText text={"AI generation job"} /></h5><span className='text-12 text-neutral-400'>{selectedJob.id}</span></div>
                ...
                {isDetailLoading ? <p className='text-14 text-neutral-400 mb-0'><StaticText text={"Loading..."} /></p> : (
                  <>
                    <div className='row gy-3'>
                      <div className='col-md-4'><div className='border border-neutral-30 rounded-8 p-14'><span className='text-12 text-neutral-400 d-block'><StaticText text={"Status"} /></span><AdminStatusBadge status={selectedJob.status} /></div></div>
                      <div className='col-md-4'><div className='border border-neutral-30 rounded-8 p-14'><span className='text-12 text-neutral-400 d-block'><StaticText text={"Generated"} /></span><strong>{selectedJob.generatedCount ?? 0} / {selectedJob.requestedCount ?? "-"}</strong></div></div>
                      <div className='col-md-4'><div className='border border-neutral-30 rounded-8 p-14'><span className='text-12 text-neutral-400 d-block'><StaticText text={"Model"} /></span><strong className='text-13'>{selectedJob.model || "-"}</strong></div></div>
                      <div className='col-md-4'><div className='border border-neutral-30 rounded-8 p-14'><span className='text-12 text-neutral-400 d-block'><StaticText text={"Subject"} /></span><strong>#{selectedJob.subjectId || "-"}</strong></div></div>
                      <div className='col-md-4'><div className='border border-neutral-30 rounded-8 p-14'><span className='text-12 text-neutral-400 d-block'><StaticText text={"Difficulty"} /></span><strong>{selectedJob.difficulty || "-"}</strong></div></div>
                      <div className='col-md-4'><div className='border border-neutral-30 rounded-8 p-14'><span className='text-12 text-neutral-400 d-block'><StaticText text={"Type"} /></span><strong>{selectedJob.questionType || "-"}</strong></div></div>
                    </div>
                    {selectedJob.errorMessage ? <div className='alert alert-danger text-14 py-10 mt-16 mb-0'>{selectedJob.errorMessage}</div> : null}
                    {["QUEUED", "RUNNING"].includes(selectedJob.status) ? <p className='text-13 text-neutral-400 mt-16 mb-0'><StaticText text={"Status refreshes automatically every 3 seconds."} /></p> : null}
                  </>
                )}
              </div>
            </div>
